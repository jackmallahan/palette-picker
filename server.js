...
app.get('/api/v1/projects', (request, response) => { //specifies what endpoint to go to
	database('projects')
		.select() //accessing the defined 'projects' database
...
app.post('/api/v1/projects', (request, response) => { //specifies what endpoint to go to
	const name = request.body; //the fetch call should include a body with the name of the project

	if (!name.name) { //if there is no body included in fetch call, returns an error
	...
	database('projects') //accesses 'projects' database
		.insert(name, '*') //inserts the request body obj with all properties
		.then(projectObj => {
...
app.post('/api/v1/palettes', (request, response) => { //specifies what endpoint to go to
	...
	if (!palette.name && !palette.projectId) {
		return response.status(422).send({ //if the fetch call doesn't include a palette name and foreign Id, it returns a 422 error
	...
	database('palettes') //accesses 'palettes' database
		.insert(palette, '*') //inserts the request body object with all properties
...
app.get('/api/v1/palettes', (request, response) => { //specifies what endpoint to go to
	database('palettes')
...
app.delete('/api/v1/palettes/:id', (request, response) => { //specifies what endpoint to go to
